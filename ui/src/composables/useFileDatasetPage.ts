...
export function useFileDatasetPage<D extends DatasetWithFileData>(config: FileDatasetPageConfig) {
  const app = useApp();
  ...
  const rowData = computed(() =>
    Object.entries(dataset.content.data).map(([sampleId, data]) => ({
  ...
  const columnDefs = computed(() => {
    const res: ColDef<FileDatasetRow>[] = [makeRowNumberColDef(), agSampleIdColumnDef(app)];
    ...
    res.push({
      ...
      cellRendererParams: {
        resolveProgress: (fileHandle: ImportFileHandle | undefined) => {
          ...
          else return app.progresses[fileHandle];
        },
      },
      ...
      cellRendererSelector: (params) =>
        params.data?.sample
          ...
          : undefined,
      valueGetter: (params) =>
        params.data?.sample ? dataset.content.data[params.data.sample] : undefined,
      valueSetter: (params) => {
