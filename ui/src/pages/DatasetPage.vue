...
import H5DatasetPage from "./H5DatasetPage.vue";
...
<template>
  <PlBlockPage>
    ...
    <template #append>
      <PlBtnSecondary :disabled="true" justifyCenter size="small">
        {{ datasetTypeLabel }}
      </PlBtnSecondary>
      <PlBtnGhost icon="delete-bin" @click="() => (deleteModalOpen = true)"
        >Delete Dataset</PlBtnGhost
      >
      <PlBtnGhost
        v-if="dataset.content.type !== 'TaggedFastq'"
        icon="dna-import"
        @click.stop="() => (app.showImportDataset = true)"
      >
        Add Files
      </PlBtnGhost>
    </template>
    ...
    <template v-else-if="dataset.content.type === 'H5'">
      <H5DatasetPage />
    </template>
